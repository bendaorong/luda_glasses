 ...
 * 
 */
var enterpriseRegisterApp = angular.module("enterpriseRegisterApp", [ 'globalLanguageApp', 'ngAnimate' , 'globalConstantApp' ]);
enterpriseRegisterApp.value("steps", [ {
	"name" : "REGISTER_INFO"
}, {
...
enterpriseRegisterApp.controller('enterpriseRegisterController', function($scope, steps, $http, countries, $http) {
	$scope.steps = steps;
	$scope.currentStep = 0;
	...
	$scope.submit = function(){
		...
		$http.post("../rest/register/enterprise", $scope.register).success(function(data){
			...
			window.location.href = window.location.origin + "/richmen/login/notpersonal?type=Z002&seatNumber=" + data.seatNumber;
		}).error(function(data){
			console.log(data);
			$scope.isLoading = false;
