...
angular.module("financialSeatManageApp").controller('infoController', function($scope, $http, countries) {

	function init() {
		$http({
			method : 'GET',
			url : 'seat_manage/financial/fetchBasicInfo'
		...
		$http({
			method : 'GET',
			url : 'seat_manage/financial/fetchIdentifications'
		}).success(function(data, status, headers, config) {
		...
		$http({
			method : 'GET',
			url : 'seat_manage/financial/fetchAddresses'
		}).success(function(data, status, headers, config) {
			$scope.addresses = {};
			console.log(data);
...
*/	
	...
	$scope.submitBasicInfo = function(){
		$scope.editBasicInfo = false;		
    	$http.post("seat_manage/financial/updateBasicInfo",
    			{
	...
	$scope.submitOtherInfo = function(){
		$scope.editOtherInfo = false;		
    	$http.post("seat_manage/financial/updateOtherBasicInfo",
    			{
	...
	$scope.submitAddress = function(){
		...
		if($scope.seatAddress.addressId == undefined ){
	    	$http.post("seat_manage/financial/addAddresses",
	    			{
		...
		if($scope.seatAddress.addressId != undefined && $scope.seatAddress.addressId != ""){
	    	...
	    	$http.post("seat_manage/financial/updateAddresses",
	    			{
	...
	$scope.removeAddress = function(addressId){
    	$http.post("seat_manage/financial/removeAddress",
    			{
     				"addressId": addressId
    			}
    		).success(function(data, status, headers, config) {
	...
	$scope.submitId = function(){
		...
		if($scope.seatId.idSN == undefined ){
	    	$http.post("seat_manage/financial/addIdentification",
	    			{
		...
		if($scope.seatId.idSN != undefined && $scope.seatId.idSN != ""){
	    	$http.post("seat_manage/financial/updateIdentification",
	    			{
	    				"countryCode": $scope.seatId.countryCode,
	    				"idType": $scope.seatId.idType,
	    				"idNo":$scope.seatId.idNo,
	...
	$scope.removeId = function(id){
    	...
    	$http.post("seat_manage/financial/deleteIdentification",
    			{
    		      "id" : id
    			}
    		).success(function(data, status, headers, config) {
    ...
    $scope.removeAttachment = function(id){
    	...
    	$http.post("seat_manage/financial/removeAttachment",
    			{
    		      "id" : id
    			}
    		).success(function(data, status, headers, config) {
	...
	$scope.addSpecialId = function() {
		$scope.editSpecialId = true;
		$scope.seatSpecialId = {};
		//set default values
		$scope.seatSpecialId.countryCode = 'CN';
		$scope.seatSpecialId.idType = 'ZFS001';
		$scope.seatSpecialId.validFrom = new Date();
		$scope.seatSpecialId.validTo = new Date();
	}
	
	$scope.changeSpecialId = function(countryCode, idType, idSN, idNo, validFrom, validTo) {
		$scope.editSpecialId = true;
		$scope.seatSpecialId.countryCode = countryCode;
		$scope.seatSpecialId.idType = idType;
		$scope.seatSpecialId.idSN = idSN;
		$scope.seatSpecialId.idNo = idNo;
		$scope.seatSpecialId.validFrom = new Date(validFrom);
		$scope.seatSpecialId.validTo = new Date(validTo);
	}

	$scope.cancelSpecialId = function() {
		$scope.editSpecialId = false;
	};
	$scope.submitSpecialId = function(){
		$scope.editSpecialId = false;
		//添加提交
		if($scope.seatSpecialId.idSN == undefined ){
	    	$http.post("seat_manage/financial/addIdentification",
	    			{
	    				"countryCode": $scope.seatSpecialId.countryCode,
	    				"idType": $scope.seatSpecialId.idType,
	    				"idNo":$scope.seatSpecialId.idNo,
	    				"validFrom" : $scope.seatSpecialId.validFrom,
	    				"validTo" : $scope.seatSpecialId.validTo
	    			}
	    		).success(function(data, status, headers, config) {
	    	    	alert("执照添加成功");
	    			console.log(data);
		...
		//修改提交
		if($scope.seatSpecialId.idSN != undefined && $scope.seatSpecialId.idSN != ""){
	    	$http.post("seat_manage/financial/updateIdentification",
	    			{
	    				"countryCode": $scope.seatSpecialId.countryCode,
	    				"idType": $scope.seatSpecialId.idType,
	    				"idNo":$scope.seatSpecialId.idNo,
	    				"idSN": $scope.seatSpecialId.idSN,
	    				"validFrom" : $scope.seatSpecialId.validFrom,
	    				"validTo" : $scope.seatSpecialId.validTo
	    			}
	    		).success(function(data, status, headers, config) {
	...
	$scope.removeSpecialId = function(id){
		$scope.editSpecialId = false;		
		$scope.seatSpecialId = {};//如果有修改或添加数据，清空，关闭。能先提示比较好。
    	$http.post("seat_manage/financial/deleteIdentification",
    			{
    		      "id" : id
    			}
    ...
    $scope.removeSpecialAttachment = function(id){
    	//if certification form is shown, remove it first, 
    	//otherwise it may make users confusing if the same certification is being edit
    	//如果有修改或添加数据，清空，关闭。能先提示比较好。
		$scope.editSpecialId = false;		
